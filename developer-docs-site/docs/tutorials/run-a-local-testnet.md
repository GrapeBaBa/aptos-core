...
You can run a local testnet of the Aptos Blockchain. This network runs independently of the Aptos ecosystem and is primarily for testing and development purposes.
...
2. The Docker is particularly useful for building services on top of the Aptos Blockchain or the Aptos Framework, as there is no build overhead and the ledger persists across network restarts.
...
* `Aptos root key path`: The root key (also known as the mint or faucet key) controls the account that can mint coins. Available in the docker compose folder under `aptos_root_key`.
...
* `ChainId`: The chain id uniquely distinguishes this blockchain from other blockchains.
